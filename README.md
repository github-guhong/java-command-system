...
​	按键监听处理器，我希望在扩展的时候不需要修改核心逻辑，所以，把键盘监听抽象出来，只需要实现这个处理器就能实现对某个按键
...
- 优化
    ...
    - **优化代码（开新分支）**。
        - 现在所有的操作都写在run方法里，这样太臃肿了。
        - 优化命名。CommandJob -> Command
        - 优化配置文件分组。系统配置放到system分组下
        - 
    - **优化整体代码**
    - **优化build命令**。现在build命令会直接覆盖原来的配置和数据文件，这会导致之前的数据直接丢失
- bug
